...
class StratifiedSample(object) :
    ...
    def __call__(self, 
        ...
        if self.n_samples == 1 :    # 一个sample 有几帧
            i = np.random.randint(n_frames)  # 随便返回一个帧
            return pts[i:i+1]
        ...
        if n_frames < self.n_samples :
            ...
            return pts
        # 该数组mask表示从0到n_frames - 1（不包括n_frames-1）均匀间隔的数列
        mask = np.floor(np.linspace(0, n_frames-1, self.n_samples)).astype(np.int32)      # drop掉一些帧
        return pts[mask]
