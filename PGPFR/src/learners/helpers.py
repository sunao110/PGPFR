...
class Teacher_v2(nn.Module):
    ...
    def __init__(self, solver, sample_shape, iters, class_idx, num_inverted_class, num_known_classes, deep_inv_params,
    ...
    def generate_inverted_samples(self, class_size, inversion_batch_size, dataloader_batch_size, log_dir_task,
                                  inverted_sample_dir):
        ...
        targets_list = [i for i in
                        range(self.num_known_classes, self.num_known_classes + self.num_inverted_class)] * class_size
        ...
        for batch_id in range(num_batches):
            ...
            inputs = torch.normal(self.inv_mean, self.inv_std, size=(
            num_samples_batch, self.sample_shape[1], self.sample_shape[2], self.sample_shape[3])).cuda()
            ...
            targets = torch.LongTensor(targets_list[
                                       batch_id * inversion_batch_size:batch_id * inversion_batch_size + num_samples_batch]).cuda()
            acc_meter.reset()
        ...
        for batch_id in range(num_batches):
            if batch_id == num_batches - 1:
                num_samples_batch = size - (batch_id * num_samples_batch)
            inputs_list = self.inverted_pts[
                          batch_id * inversion_batch_size:batch_id * inversion_batch_size + num_samples_batch]
            targets_list = self.inverted_targets[
                           batch_id * inversion_batch_size:batch_id * inversion_batch_size + num_samples_batch]
            inputs = torch.zeros(
                size=(num_samples_batch, self.sample_shape[1], self.sample_shape[2], self.sample_shape[3])).cuda()
        ...
        self.inverted_dataloader = DataLoader(self.inverted_dataset, batch_size=dataloader_batch_size, shuffle=True,
    ...
    def sample(self):
        ...
        try:
            data = next(self.inverted_iterator)
            inputs, targets, outputs = data['pts'], data['label'], data['output']
